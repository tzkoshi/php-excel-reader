...
class SpreadsheetReader
{
    ...
    function rowhidden($row, $sheet = 0): bool
    {
        if (!isset($this->rowInfo[$sheet][$row])) {
            return false;
        }
    ...
    var $_defaultFormat = Globals::SPREADSHEET_EXCEL_READER_DEF_NUM_FORMAT;
    ...
    function _parse()
    {
        ...
        $code = Utils::v($data, $pos);
        $length = Utils::v($data, $pos + 2);
        $version = Utils::v($data, $pos + 4);
        $substreamType = Utils::v($data, $pos + 6);

        $this->version = $version;

        if (($version != Globals::SPREADSHEET_EXCEL_READER_BIFF8) &&
            ($version != Globals::SPREADSHEET_EXCEL_READER_BIFF7)) {
            return false;
        }

        if ($substreamType != Globals::SPREADSHEET_EXCEL_READER_WORKBOOKGLOBALS) {
            return false;
        }
        ...
        while ($code != Globals::SPREADSHEET_EXCEL_READER_TYPE_EOF) {
            switch ($code) {
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_SST:
                    $spos = $pos + 4;
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = Utils::v($data, $spos);
                            $conlength = Utils::v($data, $spos + 2);
                            if ($opcode != 0x3c) {
                                return -1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = Utils::v($data, $spos);
                            $spos += 2;
                        }
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_FILEPASS:
                    return false;
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_NAME:
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    $indexCode = Utils::v($data, $pos + 4);
                    if ($version == Globals::SPREADSHEET_EXCEL_READER_BIFF8) {
                        $numchars = Utils::v($data, $pos + 6);
                        if (ord($data[$pos + 8]) == 0) {
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_FONT:
                    $height = Utils::v($data, $pos + 4);
                    $option = Utils::v($data, $pos + 6);
                    $color = Utils::v($data, $pos + 8);
                    $weight = Utils::v($data, $pos + 10);
                ...
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_PALETTE:
                ...
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_XF:
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
                    ...
                    $this->xfRecords[] = $xf;
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_NINETEENFOUR:
                    $this->nineteenFour = (ord($data[$pos + 4]) == 1);
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    ...
                    } elseif ($version == Globals::SPREADSHEET_EXCEL_READER_BIFF7) {
    ...
    function _parsesheet($spos)
    {
        ...
        if (($version != Globals::SPREADSHEET_EXCEL_READER_BIFF8) && ($version != Globals::SPREADSHEET_EXCEL_READER_BIFF7)) {
            return -1;
        }

        if ($substreamType != Globals::SPREADSHEET_EXCEL_READER_WORKSHEET) {
            return -2;
        }
        $spos += $length + 4;
        while ($cont) {
            $lowcode = ord($data[$spos]);
            if ($lowcode == Globals::SPREADSHEET_EXCEL_READER_TYPE_EOF) break;
            ...
            unset($this->rectype);
            switch ($code) {
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_DIMENSION:
                    if (!isset($this->numRows)) {
                        if (($length == 10) || ($version == Globals::SPREADSHEET_EXCEL_READER_BIFF7)) {
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_RK:
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_RK2:
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_MULRK:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                ...
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_FORMULA:
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_BOOLERR:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $string = ord($data[$spos + 6]);
                    $this->addcell($row, $column, $string);
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_STRING:
                    // http://code.google.com/p/php-excel-reader/issues/detail?id=4
                    if ($version == Globals::SPREADSHEET_EXCEL_READER_BIFF8) {
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_ROW:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_DBCELL:
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_MULBLANK:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $cols = ($length / 2) - 3;
                    for ($c = 0; $c < $cols; $c++) {
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_LABEL:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $this->addcell($row, $column, substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7]) << 8));
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_EOF:
                    $cont = false;
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_HYPER:
                    ...
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_DEFCOLWIDTH:
                    $this->defaultColWidth = ord($data[$spos + 4]) | ord($data[$spos + 5]) << 8;
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_STANDARDWIDTH:
                    $this->standardColWidth = ord($data[$spos + 4]) | ord($data[$spos + 5]) << 8;
                    break;
                case Globals::SPREADSHEET_EXCEL_READER_TYPE_COLINFO:
    ...
    function _getCellDetails($spos, $numValue, $column)
    {
        ...
        if ($type == 'date') {
            ...
            $utcDays = floor($numValue - ($this->nineteenFour ? Globals::SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : Globals::SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS));
            $utcValue = ($utcDays) * Globals::SPREADSHEET_EXCEL_READER_MSINADAY;
            $dateinfo = Utils::gmgetdate($utcValue);

            $raw = $numValue;
            $fractionalDay = $numValue - floor($numValue) + .0000001; // The .0000001 is to fix for php/excel fractional diffs

            $totalseconds = floor(Globals::SPREADSHEET_EXCEL_READER_MSINADAY * $fractionalDay);
